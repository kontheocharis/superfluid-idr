...
unelabItem _ (Def (MkDefItem n pr ty tm)) = pure . cast $ [PDef n (unelabTel pr) (unelabVal ty) $ case tm of
  Just t => unelab t
  Nothing => PName (MkName "?")]
unelabItem sig (Data (MkDataItem n pr ind)) = do 
  ctors <- lookup n . toList <$> get
  pure . cast $ [PData n (unelabTel pr) (pPis (unelabTel ind) PU) (fromMaybe (MkPFields (MkPTel [<])) ctors)]
unelabItem _ (Prim (MkPrimItem n pr ty)) = pure . cast $ [PPrim n (unelabTel pr) (unelabVal ty)]
